...
class Folder:
    ...
    def get_all_directories_with_size_less_than(self, size, collection):
        if self.size < size:
            collection.append(self)
        for child in self.folders:
            child.get_all_directories_with_size_less_than(size, collection)
...
small_folders_container = []
core.get_all_directories_with_size_less_than(100000, small_folders_container)
answer = sum(folder.size for folder in small_folders_container)
print(answer)
